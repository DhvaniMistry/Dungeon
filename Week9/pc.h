...
extern const char *equip_name[num_eq_slots];
...
class pc : public character {
  ...
  public:
   ...
   uint32_t wear(uint32_t slot);
   uint32_t remove(uint32_t slot);
   uint32_t drop(dungeon *d, uint32_t slot);
   uint32_t destroy(uint32_t slot);
   uint32_t pickup(dungeon *d);
