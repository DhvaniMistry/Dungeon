...
const char *equip_name[num_eq_slots] = {
    "weapon", "offhand", "ranged", "light",  "armor",   "helmet",
    "cloak",  "gloves",  "boots",  "amulet", "lh ring", "rh ring"};
...
uint32_t pc::wear(uint32_t slot) {
 object *tmp;
 uint32_t i;
 ...
 if (eq[i] && ((eq[i]->get_type() == objtype_RING) && !eq[i + 1])) {
  i++;
 }
...
uint32_t pc::remove(uint32_t slot) {
 if (!eq[slot] || !in[slot]->is_removable() || !has_open_inventory_slot()) {
...
uint32_t pc::drop(dungeon *d, uint32_t slot) {
...
uint32_t pc::destroy(uint32_t slot) {
...
uint32_t pc::pickup(dungeon *d) {
 ...
 while (has_open_inventory_slot() &&
  ...
  in[get_first_open_inventory_slot()] = from_pile(d, position);
 }

 for (o = d->objmap[position[dim_y]][position[dim_x]]; o; o = o->get_next()) {
