...
#define MAX_MOVE_ATTEMPTS 50 // Define a reasonable limit

thread_local std::mt19937 gen(std::random_device{}());
std::uniform_int_distribution<> distrib_x(0, 80); // Adjust ranges
std::uniform_int_distribution<> distrib_y(0, 24); // based on map size

void npc_next_pos_rand(dungeon *d, npc *c, pair_t next) {
    pair_t n;
    int attempts = 0;
    do {
        n[dim_y] = distrib_y(gen); // Use the new random number generator here
        n[dim_x] = distrib_x(gen); // And here
        attempts++;
...
//     if (n[dim_y] < 0)           n[dim_y] = 0;
//     if (n[dim_y] >= max_y)  n[dim_y] = max_y - 1;
//     if (n[dim_x] < 0)           n[dim_x] = 0;
//     if (n[dim_x] >= max_x)  n[dim_x] = max_x - 1;
...
static void npc_next_pos_erratic(dungeon *d, npc *c, pair_t next) {
  if (c->erratic_cooldown > 0) {
      c->erratic_cooldown--;
      return; // Skip move this turn
  }

  if (rand() & 1) {
      npc_move_func[c->characteristics & 0x7](d, c, next); // Call appropriate move function
  } else {
      npc_next_pos_00(d, c, next); // Or some other default move
  }
  c->erratic_cooldown = 5; // Set cooldown after erratic move
}
