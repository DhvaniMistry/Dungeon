...
void render_movement_cost_map(dungeon_t *d)
{
    ...
    for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) 
    {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) 
        {
            if (p[dim_x] == d->pc.position[dim_x] && p[dim_y] == d->pc.position[dim_y]) 
            {
                putchar('@');
            } 
            else 
            {
                // Check if the tile is a wall with hardness 255
                ...
                {
                    // Print the movement cost based on the hardness
    ...
    for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) 
    {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) 
        {
            ...
            else 
            {
                ...
                case ter_floor_room:
                case ter_floor_hall:
                    // Print 'X' for unreachable tiles (infinity)
...
int stairs(dungeon_t *d)
{
    ...
    // Randomly decide the type of stairs (up or down)
    type = rand_range(0, 1);
    room = rand_range(1, d->num_rooms - 1); // Random room selection

    // Randomly select a position within the chosen room
    ...
    // Place the stairs in the dungeon
    if (type == 0) 
    {
