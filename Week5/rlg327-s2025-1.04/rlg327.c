...
void printMonsters(dungeon_t *d, int start, monster_t monsters[], int count) {
  clear();
  refresh();
  move(0, 0);
  
  int line = 2;
  int end = (MAX_MONSTERS < count - start) ? (start + MAX_MONSTERS - 1) : (count - 1);
  
  mvprintw(0, 30, "Monsters List");

  for (int i = start; i <= end; i++, line++) {
      int y_offset = monsters[i].position[dim_y] - d->pc.position[dim_y];
      int x_offset = monsters[i].position[dim_x] - d->pc.position[dim_x];

      mvprintw(line, 30, "%c, %d %s %d %s",
               monsters[i].symbol,
               abs(y_offset), (y_offset >= 0) ? "north" : "south",
               abs(x_offset), (x_offset >= 0) ? "east" : "west");
...
void showMonsters(dungeon_t *d, monster_t monsters[], int input, int count) {
  int scroll = 0;
  printMonsters(d, scroll * MAX_MONSTERS, monsters, count);

  while (input != 27) { // Escape key to exit
      input = getch();
      ...
      switch (input) {
          ...
          case KEY_DOWN:
              if ((scroll + 1) * MAX_MONSTERS < count) { 
                  scroll++;
                  printMonsters(d, scroll * MAX_MONSTERS, monsters, count);
              }
              break;
