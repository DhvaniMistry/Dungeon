...
typedef struct dungeon {
  ...
  uint32_t nontunnelingMap[DUNGEON_Y][DUNGEON_X];
	uint32_t tunnelingMap[DUNGEON_Y][DUNGEON_X];
...
#define distance(pair) (d->hardness[pair[dim_y]][pair[dim_x]]/85 +1)

void Dijkstra_for_tunneling_monsters(dungeon_t *d) {
    ...
    for (y = 0; y < DUNGEON_Y; y++) {
        for (x = 0; x < DUNGEON_X; x++) {
            path[y][x].cost = INT_MAX; 
            d->nontunnelingMap[y][x] = 0;
        }
    }

    path[d->pc[dim_y]][d->pc[dim_x]].cost = 0;

    heap_init(&priority_queue, corridor_path_cmp, NULL);

    for (y = 0; y < DUNGEON_Y; y++) {
        for (x = 0; x < DUNGEON_X; x++) {
            if (d->map[y][x] != ter_wall_immutable) { 
                path[y][x].hn = heap_insert(&priority_queue, &path[y][x]);
            } else {
                path[y][x].hn = NULL; 
            }
        }
    ...
    while (true) {
        corridor_path_t *current = heap_remove_min(&priority_queue); 
        if (!current) break;

        d->nontunnelingMap[current->pos[dim_y]][current->pos[dim_x]] = current->cost;

        for (int dy = -1; dy <= 1; dy++) {
            for (int dx = -1; dx <= 1; dx++) {
                if (dy == 0 && dx == 0) continue;

                uint32_t ny = current->pos[dim_y] + dy;
                uint32_t nx = current->pos[dim_x] + dx;

                if (ny >= DUNGEON_Y || nx >= DUNGEON_X || !path[ny][nx].hn) continue;

                int new_cost = current->cost + distance(current->pos);

                if (path[ny][nx].cost > new_cost) {
                    path[ny][nx].cost = new_cost;
                    path[ny][nx].from[dim_y] = current->pos[dim_y];
                    path[ny][nx].from[dim_x] = current->pos[dim_x];
                    heap_decrease_key_no_replace(&priority_queue, path[ny][nx].hn);
                }
            }
        }
    }
...
void Dijkstra_for_nontunneling_monsters(dungeon_t *d) {
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X]; 
  static int initialized = 0;
  heap_t priority_queue;
  uint32_t y, x;
  ...
  while (true) {
      ...
      d->tunnelingMap[current->pos[dim_y]][current->pos[dim_x]] = current->cost;
...
void render_dijkstra_tunneling(dungeon_t *d)
{

	printf("\nDijkstra's Map for Tunneling Monsters.\n");
	int y, x;
		...
		for (x = 0; x < DUNGEON_X; x++) {
			switch (mapxy(x,y)) {
				...
				default:
					...
					if(d->nontunnelingMap[y][x] != 0){
						printf("%d", d->nontunnelingMap[y][x] % 10);
						break;
					}else{
						putchar(' ');
					}
...
void render_dijkstra_nontunneling(dungeon_t *d)
{
	printf("\nDijkstra's Map for Non-Tunneling Monsters.\n");
		...
		for (int x = 0; x < DUNGEON_X; x++) {
			switch (mapxy(x,y)) {
				...
				default:	
					...
					if(d->tunnelingMap[y][x] != 0){
						printf("%d", d->tunnelingMap[y][x] % 10);
					}else
						putchar(' ');
			}
    	}
...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  Dijkstra_for_tunneling_monsters(&d);
	Dijkstra_for_nontunneling_monsters(&d);

	render_dijkstra_nontunneling(&d);
	render_dijkstra_tunneling(&d);
