...
uint32_t parse_descriptions(dungeon_t *d)
{
  ...
  std::string monster_file = "description_files/monster_desc.txt";
  f.open(monster_file.c_str());
  ...
  std::string object_file = "description_files/object_desc.txt";
  f.open(object_file.c_str());
  //f.open(file.c_str());
...
npc *monster_description::gen_monster()
{
  npc *m;
  m = new npc();

  m->name = this->name.c_str();
  m->description = this->description.c_str();
