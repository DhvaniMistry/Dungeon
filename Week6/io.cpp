...
static bool fog_free_mode = true;
...
static character *io_nearest_visible_monster(dungeon_t *d)
{
  ...
  for (count = 0, y = 1; y < DUNGEON_Y - 1; y++) {
    for (x = 1; x < DUNGEON_X - 1; x++) {
      if (d->character_loc[y][x] && d->character_loc[y][x] != &d->pc) {
        c[count++] = d->character_loc[y][x];
      }
    }
  }
...
//   for (y = 0; y < 21; y++) {
//     for (x = 0; x < 80; x++) {
//       if (d->character_loc[y][x]) {
//         mvaddch(y + 1, x, d->character_loc[y][x]->symbol);
//       } else {
//         switch (mapxy(x, y)) {
//         case ter_wall:
...
uint32_t io_teleport_rand(dungeon_t *d)
{
  /* Just for fun. */
  pair_t dest;
  ...
  d->character_loc[d->pc.position[dim_y]][d->pc.position[dim_x]] = NULL;
  d->character_loc[dest[dim_y]][dest[dim_x]] = &d->pc;
...
static void io_scroll_monster_list(char (*s)[41], uint32_t count)
{
  uint32_t offset;
  uint32_t i;
                                     ...
                                     uint32_t count)
{
  ...
  s = (char(*)[41])malloc(count * sizeof (*s));

  mvprintw(3, 19, " %-40s ", "");
  /* Borrow the first element of our array for this string: */
  snprintf(s[0], 41, "You know of %d monsters:", count);
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");

  for (i = 0; i < count; i++) {
    snprintf(s[i], 41, "%16s%c: %2d %s by %2d %s",
      ...
      for (x = 0; x < DUNGEON_X; x++) {
          if (d->character_loc[y][x]) {
              ...
              switch (mapxy(x, y)) {
                  case ter_wall:
                  case ter_wall_immutable:
                      mvaddch(y + 1, x, ' '); // or '#', depending on what rock looks like
                      break;
                  case ter_floor:
                  case ter_floor_room:
                      mvaddch(y + 1, x, '.');
      ...
      for (x = 0; x < DUNGEON_X; x++) {
          if (d->map_in_sight[y][x]) {
              if ((pc_x - 3 <= (int) x && (int) x <= pc_x + 3) &&
                  ...
                  mvaddch(y + 1, x, ' '); // or '~' for unexplored areas
              }
          } else {
              mvaddch(y + 1, x, ' '); // or '~' for unseen areas
          }
      }
  }
    mvprintw(23, 1, "PC position is (%2d,%2d).",
