...
void move_character(dungeon_t *d, character *c, pair_t next)
{
  if (charpair(next) &&
    ...
    d->character_loc[c->position[dim_y]][c->position[dim_x]] = NULL;
    c->position[dim_y] = next[dim_y];
    c->position[dim_x] = next[dim_x];
    d->character_loc[c->position[dim_y]][c->position[dim_x]] = c;
  }
}
...
void do_moves(dungeon_t *d)
{
  ...
  while (pc_is_alive(d) &&
    ...
    if (!c->alive) {
      if (d->character_loc[c->position[dim_y]][c->position[dim_x]] == c) {
        d->character_loc[c->position[dim_y]][c->position[dim_x]] = NULL;
      }
      if (c != &d->pc) {
        event_delete(e);
...
uint32_t teleport_pc(dungeon_t *d, uint32_t dir)
{
  ...
  {
    move_character_all(d, &d->pc, next);
    dijkstra(d);
    dijkstra_tunnel(d);
