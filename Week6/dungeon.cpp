...
static int create_cycle(dungeon_t *d)
{
  ...
  for (i = max = 0; (uint32_t)i < d->num_rooms - 1; i++) {
    for (j = i + 1; (uint32_t)j < d->num_rooms; j++) {
...
void delete_dungeon(dungeon_t *d)
{
  ...
  memset(d->character_loc, 0, sizeof (d->character_loc));
}
...
void new_dungeon(dungeon_t *d)
{
  ...
  place_pc(d);
  d->character_loc[d->pc.position[dim_y]][d->pc.position[dim_x]] = &d->pc;
